// ────────────────────────────────────────────────────────────────────────
// Inner Ops — Firestore admin helpers
//
// Pass 3 New Finding 6 remediation: split out from firebaseUtils.js so the
// admin/migration/duplicate surface is no longer reachable from the normal
// data-layer module. These helpers issue UNFILTERED queries and bulk
// rewrite/delete operations across collections, and must only be loaded
// inside the dev-only debug branch in Dashboard.jsx.
//
// IMPORT POLICY: this file may ONLY be imported by:
//   - src/pages/Dashboard.jsx (dynamic import inside an `import.meta.env.DEV`
//     branch)
//   - test files
//
// Any other import is a regression. Future enforcement: add an ESLint rule
// or a Vite build-time guard that fails the build if `firebaseAdmin` is
// statically imported by user-facing code.
// ────────────────────────────────────────────────────────────────────────
