    ...
    <div className="space-y-3 mb-8 animate-fade-in-up">
      {killList && (
        <div className="border border-[#ef4444]/30 bg-[#ef4444]/5 rounded-2xl p-4">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div>
              <p className="text-[#ef4444] text-xs uppercase tracking-widest mb-1">Kill List Signal</p>
              <p className="text-white text-sm font-medium">{killList.targetTitle}</p>
            </div>
            <button
              onClick={onDismissKillList}
              className="text-[#3a3a3a] hover:text-[#8a8a8a] transition-colors text-xs flex-shrink-0 mt-0.5"
              title="Dismiss"
            >
              ✕
            </button>
          ...
          <p className="text-[#3a3a3a] text-xs italic mb-3 border-l-2 border-[#ef4444]/20 pl-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={onDismissKillList}
              className="px-4 py-1.5 text-[#3a3a3a] hover:text-[#5a5a5a] text-xs transition-colors"
            >
              Dismiss
            </button>
          </div>
      ...
      {relapseRadar && (
        <div className="border border-[#f59e0b]/30 bg-[#f59e0b]/5 rounded-2xl p-4">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <p className="text-[#f59e0b] text-xs uppercase tracking-widest">Relapse Radar</p>
          ...
          <p className="text-[#3a3a3a] text-xs italic mb-3 border-l-2 border-[#f59e0b]/20 pl-2">
            "{relapseRadar.evidenceFromEntry}"
          </p>
          {relapseRadar.relatedKillTarget && (
            <p className="text-[#5a5a5a] text-xs mb-3">
              Connected to Kill List target: <span className="text-[#8a8a8a]">{relapseRadar.relatedKillTarget}</span>
            </p>
          )}
          <div className="flex gap-2">
            <button
